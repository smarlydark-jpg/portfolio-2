🎮 QA & Game Testing Portfolio

Mahoney Ejeh

👋 About This Repository

This repository showcases my entry-level Quality Assurance and Game Testing skills through hands-on practice projects. It focuses on manual testing, bug reporting, and clear documentation, using industry-standard tools and processes.

My goal is to demonstrate how I identify issues, reproduce bugs, and communicate findings clearly to developers and QA teams.

🧪 What This Portfolio Demonstrates

Manual game and software testing

Bug identification and reproduction

Writing clear, structured bug reports

Understanding of QA workflows

Attention to detail and problem-solving

🛠 Tools & Technologies

Jira – Bug tracking and issue management

GitHub – Documentation and version control

Microsoft Word – Test cases and reports

PC Testing Environment

Basic HTML/CSS (for understanding UI behavior)

📂 Repository Contents

This portfolio may include:

📄 Bug Reports

Steps to reproduce

Expected vs actual results

Severity and priority levels

🧾 Test Cases

Functional and usability testing scenarios

Positive and negative test cases

🎯 Testing Notes

Observations during gameplay

UI/UX issues

Performance or logic issues

🐞 Sample Bug Report Format
Title: Character gets stuck after jumping near wall

Environment:
- Platform: PC
- Build: v1.0

Steps to Reproduce:
1. Launch the game
2. Load Level 2
3. Jump repeatedly near the left wall

Expected Result:
Character should land and move freely

Actual Result:
Character becomes stuck and cannot move

Severity:
Medium

🎯 Why QA Matters to Me

I enjoy finding issues that improve user experience and gameplay quality. QA allows me to combine my attention to detail with my passion for video games while contributing meaningfully to a team’s success.

📌 Career Goal

I am seeking an entry-level Game Tester or QA Tester position where I can grow my skills, learn from experienced QA professionals, and contribute to delivering polished, high-quality games.

📫 Contact

Email: mahoneyegwurube@gmail.com

Location: Montreal, QC
GitHub: github.com/smarlydark-jpg

## MY PROJECTS

<a href="https://mahoneyegwurube.atlassian.net/jira/software/projects/GJ/boards/67">Bug reports 1</a>

<a href="https://mahoneyegwurube.atlassian.net/jira/software/projects/OG/boards/68">Bug reports 2</a>

<a href="https://1drv.ms/x/c/251cf9fb3b3e8caa/IQC6reYsNijOSpS1AJ0qKw1XATNEOGy_bdqWB9kbifG6ej0?e=aah2cE">Test cases</a>

<a href="Test Summary.pdf">Test summary</a>








## CONTACT DETAILS

<table>
  <tbody>
    <tr>
      <td>📧</td>
      <td><a href="mailto:mahoneyegwurube@gmail.com">My gmail</a></td>
    </tr>
    <tr>
      <td>📞</td>
      <td>438-926-6022</td>
    </tr>
    <tr>
      <td>📍</td>
      <td>1700 Rue Viola-Desmond
        Montréal, QC  H8N 0H1</td>
    </tr>
    <tr>
      <td>⬇️</td>
      <td><a href="https://github.com/smarlydark-jpg/portfolio-2/blob/main/CV2.jpg?raw=true">Download my CV</a></td>
    </tr>
    <tr>
      <td>🌐</td>
      <td><a href="https://www.linkedin.com/feed/">LinkedIn</a></td>
    </tr>
  </tbody>
</table>
